...
- Payments: **the payment gateway** (Orders API via raw cURL — no Composer requirement; Checkout.js on the front; ticket issued from the **webhook**; QR is HMAC-signed and single-use).
...
Follow the exact project structure, DB schema, and endpoint contracts in `BUILD_SPEC.md` Sections 4–6. Keep all secrets in a git-ignored `api/config.php` (commit `api/config.sample.php` only). Ask me for the payment gateway keys and anything in "Open items" (Section 10) you need.
